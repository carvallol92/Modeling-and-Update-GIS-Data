...
def IntoInputs():
    ...
    N8 = pd.read_excel('1_entrada/'+n_inputF)
    table_id = pd.read_excel('table_id.xls')
    
        
    if mes in list_month and len(año) == 4:
        
        ProcesingDataFrame(N8,año,mes,n_input,table_id)
...
def ProcesingDataFrame(N8,año,mes,n_input,table_id):
    
    print('Procesando datos...')

    #Eliminate origen format
    N8 = N8.rename(columns=N8.iloc[3])
    N8 = N8.drop([0,1,2,3,4])
    N8.columns = [
    ...
    N8.update(updateDelito)   
    N8 = N8.drop(ep_row)

    N8[['establecimiento_penitenciario','delito']] = N8[['establecimiento_penitenciario','delito']].astype('category')
    N8[['total','hombres','mujeres',]] = N8[['total','hombres','mujeres',]].astype('int')

    N8['año'] = int(año)
    N8['mes'] = mes
    N8['id_mes'] = ReclassMes(mes)
    N8['fecha'] = ReclassFecha(mes,año)

    # Merge de Tablas
    N8_Done = pd.merge (N8,table_id, on='establecimiento_penitenciario')

    if N8_Done.isnull().values.sum() == 0:
    ...
    N8_Done.to_excel('2_salida/'+n_input+'_procesado.xlsx', sheet_name='data')
    ...
    GetUpdateService(gis,N8,año,mes,N8_Done)


# In[6]:


def GetUpdateService(gis,N8,año,mes,N8_Done):
    ...
    N8_service = fs_lyr.query().sdf

    N8_update = []
    N8_template = N8_Done.to_dict('index')[0].copy()

    for row in N8_Done.iterrows():

        template_append = N8_template.copy()
        ...
        N8_update.append({"attributes":template_append})
    ...
    fs_lyr.edit_features(adds = N8_update)
